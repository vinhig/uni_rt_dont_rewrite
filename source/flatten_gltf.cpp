...
void flatten_gltf_node(const tinygltf::Node &node, const tinygltf::Model &model,
  ...
  flat.matrix.clear();
  std::copy(glm::value_ptr(transform), glm::value_ptr(transform) + 16,
...
void flatten_gltf(tinygltf::Model &model) {
  ...
  std::copy(flattened_nodes.begin(), flattened_nodes.end(),
            std::back_inserter(model.nodes));
